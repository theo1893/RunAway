...
-- Default values (used to merge with saved variables)
local RunAway_defaults = {
    -- Addon enabled state
    enabled = true,
    -- Debug mode
    debug = false,
...
-- Initialize saved variables with defaults (merge missing keys)
-- In vanilla WoW, saved variables are loaded before addon code runs
if not RunAway_db then
    RunAway_db = {}
end

for key, value in pairs(RunAway_defaults) do
    if RunAway_db[key] == nil then
        RunAway_db[key] = value
    end
end
